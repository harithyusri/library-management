...
import { ClipboardList, UserIcon, BriefcaseIcon, LockIcon, PhoneIcon, MailIcon, Calendar } from 'lucide-vue-next'
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Edit Staff Member', href: route('admin.staffs.edit', props.staff.id) },
]
...
const form = reactive({
    ...
    role: props.staff.roles?.[0]?.name ?? 'librarian',
    employee_id: props.staffProfile?.employee_id || '',
    hire_date: props.staffProfile?.hire_date ? new Date(props.staffProfile.hire_date).toISOString().split('T')[0] : '',
    department_id: props.staffProfile?.department_id?.toString() || 'none',
    library_id: props.staffProfile?.library_id?.toString() || '',
...
const submit = () => {
    ...
    router.put(route('admin.staffs.update', props.staff.id), data as any)
}
</script>
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <Card class="border-slate-100 shadow-sm rounded-3xl overflow-hidden">
                        <div class="bg-slate-50/50 px-6 py-4 border-b border-slate-100 flex items-center gap-2">
                            ...
                            <h2 class="text-sm font-black text-slate-900 uppercase tracking-widest">Update Security</h2>
                        </div>
                        <CardContent class="p-6 space-y-6">
                            <div class="p-4 bg-amber-50 rounded-2xl border border-amber-100 flex gap-3 items-start mb-4">
                                <LockIcon class="h-4 w-4 text-amber-600 mt-1" />
                                <p class="text-xs text-amber-700 font-medium leading-relaxed">Leave password fields blank to keep the current password.</p>
                            </div>
                            <div class="grid md:grid-cols-2 gap-6">
                                <div class="space-y-2">
                                ...
                                <div class="space-y-2">
                                    <Label class="text-[10px] font-black uppercase tracking-widest text-slate-400">Confirm New Password</Label>
                                    <div class="relative">
                                        <LockIcon class="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-300" />
                        ...
                        <CardContent class="p-6 space-y-6">
                            <div class="space-y-4">
                                ...
                                <div class="space-y-2">
                                    ...
                                    <Input v-model="form.employee_id" class="rounded-xl font-mono text-xs" />
                                </div>
