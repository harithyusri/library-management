...
const form = useForm({
    ...
    genre_ids: Array.isArray(props.book.genres)
        ? props.book.genres.map(g => String(g.id))
        : [],
...
const coverImagePreview = ref<string | null>(props.book.cover_image ?? null);
...
const submitForm = () => {
    // 1. First, tell the form how to transform the data
    // (adding the _method spoofing for Laravel)
    form.transform((data) => ({
        ...data,
        _method: 'PUT',
    })).post(route('admin.books.update', props.book.id), { // 2. Then chain the .post()
        preserveScroll: true,
        forceFormData: true,
        onSuccess: () => {
                ...
                <div class="rounded-xl border border-border bg-background p-6">
                    ...
                    <div class="grid gap-6 md:grid-cols-2">
                                        ...
                                        <div class="flex flex-wrap gap-1">
                                            ...
                                            <template v-else>
                                                <Badge v-for="id in form.genre_ids" :key="id" variant="secondary"
                                                    ...
                                                    {{props.genres.find(g => g.id === Number(id))?.name ?? 'Unknown'}}
                                                </Badge>
                                            ...
                                            <CommandGroup>
                                                <CommandItem v-for="genre in props.genres" :key="genre.id"
                                                    :value="genre.name" @select="() => {
                                                        const idAsString = String(genre.id);
                                                        const index = form.genre_ids.indexOf(idAsString);

                                                        if (index > -1) {
                                                            form.genre_ids.splice(index, 1);
                                                        } else {
                                                            form.genre_ids.push(idAsString);
                                                        }
                                                    }">
                                                    <CheckIcon :class="cn(
                                                        'mr-2 h-4 w-4',
                            ...
                            <Dialog v-model:open="isPublisherDialogOpen">
                                <DialogContent class="sm:max-w-[425px]">
                                    <form @submit.prevent="submitPublisher">
                                        ...
                                        <DialogFooter>
                                            <Button type="button" variant="outline"
                                                @click="isPublisherDialogOpen = false">
                                                Cancel
                                            </Button>
                        ...
                        <div class="space-y-2">
                            ...
                            />
                            <p v-if="form.errors.published_year" class="text-xs text-destructive font-medium">
